...
export class OneBotHttpUpdate extends plugin {
  constructor() {
    super({
      name: "OneBotHttpUpdate",
      dsc: "OneBotHttp-Plugin 更新",
      event: "message",
      priority: 10,
      rule: [
        {
          reg: "^#[hH][tT][tT][pP](强制)?更新$",
          fnc: "update",
          permission: "master",
        },
      ],
  ...
  async runUpdate(isForce) {
    const plugin = "OneBotHttp-Plugin"
  ...
  async getLog(plugin, pluginPath) {
    ...
    const end = "https://github.com/eatyon/OneBotHttp-Plugin"
  ...
  async gitErr(error, stdout = "") {
    ...
    if (errMsg.includes("be overwritten by merge") || stdout.includes("CONFLICT")) {
      await this.reply(`${msg}\n存在冲突\n请解决冲突后再更新，或者执行 #http强制更新 放弃本地修改`)
      return
    }
